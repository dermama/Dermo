...
| API | `api/trpc.mjs` | Başvuru, yönetim parolası, token doğrulaması ve mesajlaşma |
...
`pnpm run vercel-build`, kaynak dosya `server/vercel/trpc.ts` içinden sürümlenmiş `api/trpc.mjs` paketini üretir. Bu paket tRPC, Drizzle, Neon, Resend ve token güvenliği bağımlılıklarını işlevin içine alır; `api/trpc.mjs` elle düzenlenmemelidir. Değişiklikler kaynak TypeScript dosyasında yapılmalı ve ardından bu derleme komutu çalıştırılmalıdır.
...
4. `pnpm run vercel-build` komutuyla bağımsız API paketini yenileyin, ardından GitHub’a gelen değişikliği dağıtın; Vercel Functions derlenirken `/api/trpc.mjs` görülmelidir.
